...
const HomePage: React.FC<HomePageProps & { showToast: (severity: 'success' | 'info' | 'warn' | 'error' | 'secondary' | 'contrast' | undefined, summary: string, detail: string) => void }> = ({ logout, showToast }) => {
  ...
  const handleDeleteSubject = (subjectId: string) => {
    setDeleteCategoryConfirmation({ isOpen: true, folderId: subjectId });
  };
  ...
  const confirmDeleteCategory = async () => {
    if (deleteCategoryConfirmation.folderId) {
      try {
        if (selectedFolder) {
          // This is a subject deletion
          //await api.deleteSubject(deleteCategoryConfirmation.folderId);
          setFolders(prevFolders => 
            prevFolders.map(folder => 
              folder.id === selectedFolder.id 
                ? { ...folder, subjects: folder.subjects.filter(subject => subject.id !== deleteCategoryConfirmation.folderId) }
                : folder
            )
          );
          showToast('success', 'Success', 'Subject deleted successfully');
        } else {
          // This is a category deletion
          ...
          setNotes(prevNotes => prevNotes.filter(note => note.folderName !== folders.find(f => f.id === deleteCategoryConfirmation.folderId)?.name));
          showToast('success', 'Success', 'Category deleted successfully');
        }
      } catch (err) {
        console.error('Error deleting category/subject:', err);
        setErrorPopup({ isOpen: true, message: 'Failed to delete. Please try again.' });
        showToast('error', 'Error', 'Failed to delete. Please try again.');
      }
    }
  ...
  /* TODO fix this kak, for edit notes functionality
  const handleEditNote = useCallback(async (noteId: string, newContent: string) => {
    try {
      const response = await api.editNote(noteId, { content: newContent }); // TODO fix this error and the edit notes name functionality
      if (response.status === 200) {
        setNotes(prevNotes =>
          prevNotes.map(note =>
      ...
      showToast('error', 'Error', 'Failed to update note. Please try again.');
    }
  }, [showToast]);
  */
  const handleEditSubject = (subjectId: string) => {
    // Implement the logic for editing a subject
    // This could open a popup or inline edit field
    console.log('Edit subject:', subjectId);
  };

  const handleSaveEdit = (noteId: string, newTitle: string) => {
    setFolders(prevFolders =>
      prevFolders.map(folder => ({
        ...folder,
  ...
  const handleAvatarClick = () => {
    ...
    showToast('info', 'Avatar', 'Avatar change functionality will be implemented soon');
  };
  ...
  return (
    <div className="home-page">
      ...
      message={`Are you sure you want to delete this ${selectedFolder ? 'subject' : 'category'}?`}
      ...
      {folderOptionsMenu.isOpen && (
        ...
        >
          <button 
            ...
            onClick={() => handleRenameFolder(folderOptionsMenu.folderId!)}
          >
            Rename
          </button>
          <button 
